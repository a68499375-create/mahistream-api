...
interface TrackerState {
  // Last known highest episode number per animeId.
  lastEpisodes: Record<string, number>;
  // Recently detected releases, oldest first.
    ...
    if (parsed && typeof parsed === "object") {
      const lastEpisodes =
        parsed.lastEpisodes && typeof parsed.lastEpisodes === "object"
          ? (parsed.lastEpisodes as Record<string, number>)
          : {};
      const pendingReleases = Array.isArray(parsed.pendingReleases)
        ? (parsed.pendingReleases as PendingRelease[])
        : [];
      state = { lastEpisodes, pendingReleases };
      console.log(
        `[releaseTracker] state loaded: ${Object.keys(lastEpisodes).length} tracked anime, ${pendingReleases.length} pending`
      );
    }
  } catch (e: any) {
    if (e?.code !== "ENOENT") {
...
function getMaxEpisodeNumber(episodeList: any[]): number {
  if (!Array.isArray(episodeList) || episodeList.length === 0) return 0;
  let max = 0;
...
async function pollOnce(): Promise<void> {
  ...
  const flat: { animeId: string; title: string; poster: string }[] = [];
    ...
    try {
      ...
      if (prev === 0) {
        ...
        if (state.pendingReleases.length > MAX_PENDING) {
          state.pendingReleases = state.pendingReleases.slice(
            state.pendingReleases.length - MAX_PENDING
          );
        }
        state.lastEpisodes[t.animeId] = latest;
        newDetections++;
        console.log(
          `[releaseTracker] new episode: ${release.title} ep ${latest} (was ${prev})`
        );
      }
    } catch (e) {
  ...
  scheduleSave();
  console.log(
    `[releaseTracker] poll done — ${newDetections} new releases, queue size ${state.pendingReleases.length}`
  );
}

export function getPendingReleases(sinceMs?: number): PendingRelease[] {
...
export function ackRelease(animeId: string, episode: number): boolean {
  const before = state.pendingReleases.length;
  state.pendingReleases = state.pendingReleases.filter(
    (r) => !(r.animeId === animeId && r.episodeNumber === episode)
  );
...
export async function startReleaseTracker(): Promise<void> {
  if (started) return;
  started = true;
  await loadState();
  ...
  console.log(
    `[releaseTracker] started, interval ${POLL_INTERVAL_MS / 1000}s, state file ${STATE_FILE}`
  );
}

export function stopReleaseTracker(): void {
  if (pollTimer) {
