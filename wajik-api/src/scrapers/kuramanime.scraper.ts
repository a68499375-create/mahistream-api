...
const tryFlare = async (path: string, maxTimeout = 55000): Promise<string> => {
  const allHosts = [
  ...
  for (const host of allHosts) {
    ...
    if (banUntil && banUntil > now) cooled.push(host);
    else healthy.push(host);
  }
  ...
  for (const host of candidates) {
    try {
      ...
      console.warn(`[kura] domain ${host} failed:`, (e as any)?.message || e);
    }
  }
  throw lastErr || new Error("All Kuramanime domains failed");
};

const tryFlarePost = async (path: string, postData: string, targetServer: string, maxTimeout = 55000): Promise<string> => {
  const allHosts = [
    "https://v18.kuramanime.ing",
    "https://v9.kuramanime.work",
    "https://v9.kuramanime.blog",
  ...
  for (const host of candidates) {
    try {
      ...
      lastErr = e;
      failedHosts.set(host, Date.now() + FAILURE_COOLDOWN_MS);
      console.warn(`[kura-post] domain ${host} failed:`, (e as any)?.message || e);
    }
  }
  throw lastErr || new Error("All Kuramanime domains failed in POST");
};

const tryPuppeteer = async (path: string, waitForSelector?: string, maxTimeout = 60000): Promise<string> => {
  const allHosts = [
    lastGoodHost,
  ...
  for (const host of candidates) {
    try {
      ...
      lastErr = e;
      failedHosts.set(host, Date.now() + FAILURE_COOLDOWN_MS);
      console.warn(`[kura-pup] domain ${host} failed:`, (e as any)?.message || e);
    }
  }
...
const tryFlareWithPuppeteer = async (path: string, waitForSelector?: string, timeout = 20000): Promise<string> => {
  ...
  try {
    ...
    console.warn("[kura] Failed to get cookies from FlareSolverr:", (e as any)?.message);
  }
...
const kuramanimeScraper = {
  async scrapeDOM(pathname: string, ref?: string, sanitize: boolean = false, waitForSelector?: string): Promise<HTMLElement> {
    ...
    if (pathname.includes("/episode/")) {
      ...
      try {
        ...
        if (html && html.length > 500) return parse(html, { parseNoneClosedTags: true });
      } catch (err) {
        console.warn("[kuramanimeScraper] Puppeteer for episode failed:", (err as any)?.message);
      }
    ...
    if (pathname.includes("/anime/") && !pathname.includes("/episode/")) {
      ...
      try {
        ...
        console.warn("[kuramanimeScraper] Puppeteer for anime detail failed:", (err as any)?.message);
      }
      ...
      try {
        ...
        console.warn("[kuramanimeScraper] Hybrid failed:", (err as any)?.message);
      }
      ...
      try {
        ...
        console.error("[kuramanimeScraper] All methods failed for anime detail:", (err as any)?.message);
      }
    }
    ...
    try {
      ...
      console.warn("[kuramanimeScraper] Puppeteer for other page failed:", (err as any)?.message);
    }
  ...
  async scrapeSecret(ref?: string): Promise<string> {
    try {
      ...
      console.warn("[kuramanimeScraper] Puppeteer scrapeSecret failed:", (err as any)?.message);
    }
