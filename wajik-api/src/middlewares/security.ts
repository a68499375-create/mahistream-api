...
export function corsWhitelist(req: Request, res: Response, next: NextFunction) {
  ...
  const isAllowed =
    ALLOWED_ORIGINS.length === 0 ||                           // mode dev / belum dikonfigur
...
setInterval(() => {
  ...
  for (const [key, b] of buckets) {
    if (b.resetAt < now - 10 * 60 * 1000) buckets.delete(key);
  }
}, 60 * 1000).unref?.();
...
const containsControlChars = (s: string) => /[\x00-\x1f\x7f]/.test(s);
