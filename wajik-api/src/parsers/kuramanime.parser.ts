...
const kuramanimeParser = {
    async parseCustom(path: string): Promise<any[]> {
    const document = await kuramanimeScraper.scrapeDOM(
      path,
      "", 
      false
    );
  ...
  async parseSearch(query: string, page: number = 1): Promise<any[]> {
    ...
    const pushResult = (id: string, title: string, poster: string, score: string, url: string) => {
      if (!id || !title) return;
    ...
    if (results.length === 0 && textPath) {
      ...
      if (textDoc) {
        ...
        links.forEach((a) => {
          const url = a.getAttribute("href") || "";
          const id = idFromUrl(url);
          if (!id || id.includes("/episode") || url.includes("/properties/")) return;
...
async parseGenres(): Promise<{ slug: string; title: string }[]> {
    try {
  ...
  async parseSchedule(): Promise<any[]> {
    ...
    const extractJam = (cardEl: any): string => {
      ...
      sel.forEach((s) => {
        ...
        if (el) {
          ...
          if (dt) candidates.push(String(dt));
    ...
    try {
      ...
      if (days.every((d) => (buckets[d] || []).length === 0)) {
        ...
        allCards.forEach((c) => {
          ...
          const poster = c.querySelector(".product__item__pic")?.getAttribute("data-setbg") || c.querySelector("img")?.getAttribute("data-src") || c.querySelector("img")?.getAttribute("src") || "";
          const jam = extractJam(c);
          if (id && title) (buckets[dayName] ||= []).push({ animeId: id, title, poster, jam, url });
        });
      }
  ...
  async parseAnimeDetails(id: string): Promise<any> {
    try {
      ...
      const unescapeHtml = (raw: string) => raw
        .replace(/&lt;/g, '<')
        .replace(/&gt;/g, '>')
        .replace(/&quot;/g, '"')
        .replace(/&amp;/g, '&')
      ...
      const harvestEpisodes = (rawHtml: string): number => {
        if (!rawHtml) return 0;
        ...
        epLinks.forEach(a => {
          ...
          if (epTitle.includes("Terbaru") || epTitle.includes("Terlama")) return;
          ...
          if (!epIdMatch || !epIdMatch[1]) return;
          const epId = epIdMatch[1].split("?")[0]!.split("#")[0]!;
          if (!epId || seenEpisodeIds.has(epId)) return;
          ...
          if (epNumber && seenEpisodeNumbers.has(epNumber)) return;
          if (epNumber) seenEpisodeNumbers.add(epNumber);
      ...
      if (episodeList.length === 0) {
        ...
        if (fallbackHtml) harvestEpisodes(fallbackHtml);
      }
      ...
      try {
        ...
        pageLinks.forEach(a => {
          ...
          if (m && m[1]) {
            ...
            if (n > 1) foundPages.add(n);
          }
        });

        const fetchPage = async (pageN: number): Promise<string> => {
          try {
        ...
        const fetchPageWithBudget = async (pageN: number): Promise<string> => {
    ...
    try {
      ...
      if (dlContainer) {
        ...
        for (let i = downloadLinks.length - 1; i >= 0; i--) {
          if (!downloadLinks[i].links.length) downloadLinks.splice(i, 1);
        }
      }
  ...
  async parseStreamUrl(serverId: string): Promise<string> {
    try {
      ...
      const document = await kuramanimeScraper.scrapeDOM(
        ...
        undefined,
        false,
        "#player, iframe, .player-frame, #video-player, video source"
      );

      // 1) Player Kuramadrive: <video id="player"><source src=".../720p.mp4">…
      const sourceTags = document.querySelectorAll(
        "#player source, video source, video#player source"
      );
