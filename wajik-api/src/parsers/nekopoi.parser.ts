...
const extractBgImage = ($el: Cheerio<Element>): string => {
...
function parseCard($: CheerioAPI, el: AnyNode): { id: string; title: string; image: string; type: string } | null {
  const $el = $(el as any);
  ...
  if (!anchor.length && $el.is("a")) anchor = $el;
  ...
  let rawTitle = firstNonEmpty(
    $el.find(".nk-post-title, .post-title, .entry-title, .title, h2, h3, h4").first().text(),
    anchor.attr("title"),
    anchor.text(),
  ...
  if (!image) {
    ...
    if ($bg.length) image = extractBgImage($bg);
  }
...
function collectListing(html: string) {
  const $ = load(html);
  ...
  if (results.length === 0) {
    ...
    $("a[href*='nekopoi.care/']").each((_, el) => {
      ...
      const id = slugFromHref(href);
      if (!id || id.includes("page/") || id.includes("category/") || id.includes("search/")) return;
...
const nekopoiParser = {
  ...
  parseIndexList(html: string) {
    ...
    $("a[href]").each((_, el) => {
      ...
      if (NAV_BLOCK.has(firstSeg)) return;
  ...
  parseDetail(html: string, id: string) {
    const $ = load(html);
    ...
    const title = firstNonEmpty(
      ...
      $(".nk-player-series-title").first().text(),
      $(".nk-post-header h1").first().text(),
      $(".eropost h1, .erotitle h1, .erotitle").first().text(),
      $(".post-title, .entry-title, h1.title").first().text(),
    ...
    const image = firstNonEmpty(
      $(".nk-player-series-thumb").attr("style")?.match(/url\(['"]?(.*?)['"]?\)/)?.[1],
      $(".nk-featured-img img").attr("src"),
      $(".eroimg img").attr("src"),
      $(".featured-image img, .post-thumbnail img, .thumb img").first().attr("src"),
      $('meta[property="og:image"]').attr("content"),
      $("article img").first().attr("src")
    );

    const synopsis = firstNonEmpty(
      $(".nk-post-body .konten").text(),
      $(".eroinfo p").text(),
      $(".konten").text(),
      $(".synopsis, .description, .post-content p").first().text(),
    ...
    const qualityFromText = (text?: string | null): string => {
      if (!text) return "";
    ...
    $("#nk-player .nk-player-frame, .nk-player-frame, .player-frame, [id^='nk-stream-']").each((idx, frameEl) => {
      ...
      if (/discordapp|a-ads|googlesyndication/i.test(src)) return;
      ...
      for (const c of dataCandidates) {
        ...
        if (q) { quality = q; break; }
      }
      if (!quality) quality = qualityFromText($frame.attr("id"));
      const frameId = $frame.attr("id") || "";
      const tabLabel = tabLabelMap[frameId] || "";
      if (!quality) quality = qualityFromText(tabLabel);
      if (!quality) {
        if (frameId) {
    ...
    if (streamLinks.length === 0) {
      const iframeSrc = firstNonEmpty(
        $("#player iframe").attr("src"),
        $(".embed-player iframe").attr("src"),
        $(".player iframe").attr("src"),
        $("iframe[src*='player']").attr("src"),
        $("iframe[src*='embed']").attr("src"),
        $("iframe[src*='streampoi']").attr("src"),
        $("iframe[src*='streamtape']").attr("src"),
        $("iframe[src*='vidnest']").attr("src"),
        $("iframe[src*='playmogo']").attr("src"),
        $("article iframe").first().attr("src")
      );
    ...
    const pushDownload = (quality: string, links: any[]) => {
    ...
    $(".episodelist ul li, .nk-episode-list li, .episode-list li, .eplist li").each((_, el) => {
      ...
      const href = $a.attr("href");
      const epId = slugFromHref(href);
      if (!epTitle || !epId || seenEp.has(epId)) return;
      seenEp.add(epId);
