...
const otakudesuParser = {
  ...
  async parseEpisodeDetails(document: HTMLElement, url: string): Promise<T.IEpisodeDetails> {
    ...
    let downloadElems = document.querySelectorAll(".download ul li");
    if (downloadElems.length === 0) {
      downloadElems = document.querySelectorAll(".venser ul li");
    }
    ...
    const animeId = alertInfo && alertInfo.lastElementChild ? Id(alertInfo.lastElementChild.querySelector("a")) : "";
  ...
  async parseServerDetails(serverId: string): Promise<T.IServerDetails> {
    ...
    const attempt = async (obj: Record<string, any>) => {
      const body = new URLSearchParams(obj as any);
      const server = await otakudesuScraper.scrapeServer(body.toString(), referer);
      return generateSrcFromIframeTag(Buffer.from(server.data || "", "base64").toString());
    };

    let url = await attempt(serverIdObj);

    // Nonce di Otakudesu cepat expire (rotasi per kunjungan halaman).
    // Kalau gagal, re-scrape halaman episode untuk dapat credentials + nonce
    // segar, lalu retry sekali.
    if (!url || url === "No iframe found") {
      try {
        const pathname = new URL(referer).pathname;
        const document = await otakudesuScraper.scrapeDOM(pathname);
        const credentials = [
          ...new Set(
            [...document.innerText.matchAll(/action:"([^"]+)"/g)].map((m) => m[1])
          ),
        ];
        if (credentials.length >= 2) {
          const nonceBody = new URLSearchParams({ action: credentials[1] || "" });
          const nonce = await otakudesuScraper.scrapeNonce(nonceBody.toString(), referer);
          if (nonce.data) {
            const fresh = {
              ...serverIdObj,
              nonce: nonce.data,
            ...
            url = await attempt(fresh);
          }
        }
      } catch (e) {
        console.warn("[otakudesu parseServerDetails] nonce refresh failed:", e);
      }
    }
