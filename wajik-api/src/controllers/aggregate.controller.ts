...
const readHomeCache = (): { ts: number; data: any } | null => {
  try {
...
async function buildHomeAndCache(): Promise<any> {
  ...
  const kuraCustomSafe = async (p: string) => {
    try {
      const r = await withTimeout(kuramanimeParser.parseCustom(p), 35000) as any[] | null;
  ...
  const mapKura = (it: any, fallbackEpisodes: string) => ({
    animeId: it.animeId,
    title: it.title,
    poster: it.poster,
  ...
  const kuraSeen = new Set<string>();
  const kuraOngoing: any[] = [];
  const kuraOverflow: any[] = [];

  // Page 1 -> ongoing list. Page 2-3 -> completed/overflow.
  kuraPage1.forEach((it) => {
    const k = norm(it.title);
    if (!k || kuraSeen.has(k)) return;
    kuraSeen.add(k);
...
const aggregateController = {
  async search(req: Request, res: Response) {
    ...
    try {
      ...
      const runSource = async (
        run: (q: string) => Promise<any[]>,
        timeoutMs: number,
      ): Promise<any[]> => {
        const lists = await Promise.all(
          queries.map((q) =>
            withTimeout(run(q), timeoutMs)
      ...
      const addSourceResults = (results: any[], sourceName: "otakudesu" | "kuramanime" | "nekopoi") => {
  ...
  async schedule(req: Request, res: Response) {
    try {
      let kuraSchedule: any[] = [];
      let otakuSchedule: any[] = [];
      ...
      try {
        kuraSchedule = await withTimeout(kuramanimeParser.parseSchedule(), 25000) as any[];
      } catch (e) {
        console.error("Kuramanime schedule failed", e);
      }
      try {
        const doc = await withTimeout(otakudesuScraper.scrapeDOM("/jadwal-rilis/", "https://otakudesu.blog/"), 20000) as any;
      ...
      const posterPool = new Map<string, string>();
      try {
        const [p1, p2] = await Promise.all([
          withTimeout(kuramanimeParser.parseCustom("/anime?order_by=updated&page=1"), 25000) as Promise<any[] | null>,
          withTimeout(kuramanimeParser.parseCustom("/anime?order_by=updated&page=2"), 25000) as Promise<any[] | null>,
        ]);
        [...(p1 || []), ...(p2 || [])].forEach((it: any) => {
      ...
      const findPoster = (title: string): string => {
        const k = norm(title);
        if (posterPool.has(k)) return posterPool.get(k)!;
        // Cocokkan partial — title bisa beda "Season X" / "Part Y" suffix.
        for (const [key, poster] of posterPool) {
          if (key.startsWith(k) || k.startsWith(key)) return poster;
        }
        return "";
      };
      ...
      const merged = days.map((day) => {
        ...
        (otaku?.animeList || []).forEach((it: any) => {
          const key = (it.title || "").toLowerCase().trim();
          if (!key || seen.has(key)) return;
          seen.add(key);
          list.push({
