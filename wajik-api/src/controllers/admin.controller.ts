...
async function pingUrl(url: string, label: string): Promise<{ label: string; status: string; time: number; error?: string }> {
...
const adminController = {
  ...
  async flareStatus(_req: Request, res: Response) {
    try {
      const ctrl = new AbortController();
  ...
  async cacheStats(_req: Request, res: Response) {
    const cacheSize = ((global as any).__streamCache?.size || 0);
    return res.status(200).json(setPayload(res, { data: { cacheEntries: cacheSize, cacheTTL: "5 menit" } }));
  },

  async systemInfo(_req: Request, res: Response) {
    const info: any = {
