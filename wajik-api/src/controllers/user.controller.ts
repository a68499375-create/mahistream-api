...
export const saveHistory = async (req: Request, res: Response) => {
  try {
    ...
    if (userRows.length === 0) {
      ...
      await runDb(
        `INSERT INTO users (id, uid, email, username, display_name) VALUES (?, ?, ?, ?, ?)`,
        [actualUid, actualUid, email, username, username]
      );
    } else {
    ...
    if (existing.length > 0) {
      const finalDurationSec = durationSec || (existing[0] as any).duration_seconds || 0;
      await runDb(
        `UPDATE history SET episode = ?, progress_seconds = ?, duration_seconds = ?, watched_at = CURRENT_TIMESTAMP WHERE user_id = ? AND anime_id = ?`,
        [episode, progressSec, finalDurationSec, actualUid, animeId]
      );
    } else {
      await runDb(
        `INSERT INTO history (user_id, anime_id, title, poster_url, episode, source, progress_seconds, duration_seconds) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [actualUid, animeId, title, pUrl, episode, src, progressSec, durationSec]
      );
    }
...
export const toggleBookmark = async (req: Request, res: Response) => {
  try {
    ...
    if (existing.length > 0) {
      await runDb('DELETE FROM bookmarks WHERE id = ?', [(existing[0] as any).id]);
      res.status(200).json({ message: 'Bookmark removed', isBookmarked: false });
    } else {
      await runDb(
        `INSERT INTO bookmarks (user_id, anime_id, title, poster_url, source) VALUES (?, ?, ?, ?, ?)`,
        [actualUid, animeId, title, posterUrl, source]
      );
...
export const getBookmarks = async (req: Request, res: Response) => {
  try {
    ...
    const bookmarks = await queryDb(
      `SELECT anime_id as id, title, poster_url as posterUrl, source 
       FROM bookmarks 
       WHERE user_id = ? 
  ...
  try {
    ...
    await runDb(
      `UPDATE users 
       ...
       WHERE id = ?`,
      [display_name, username, bio, picture, background_url, userId]
    );
