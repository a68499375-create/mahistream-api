...
const streamCache = new Map<string, { result: string; ts: number }>();
...
function isVideoStreamUrl(url: string): boolean {
  ...
  if (FALSE_POSITIVE_PATTERNS.some(p => lower.includes(p))) return false;
  // Exclude ad-related URLs (but be careful not to exclude "load" or "download")
  if (/[/.]ad[s]?[/.\-?]/.test(lower) || lower.includes('/adserver')) return false;
  ...
  if (/\.ts(\?|$)/i.test(url) && lower.includes('/hls/')) return true;
  ...
  if (/\/video\/[^/]+\.(mp4|m3u8|webm|ts)(\?|$)/i.test(url)) return true;
  if (/\/video\/[a-f0-9]{8,}/i.test(url) && !lower.includes('/video/manifest')) return true;
 ...
 */
async function extractBloggerStream(page: Page, browser: any): Promise<string | null> {
  try {
    ...
    try {
      ...
      let bloggerStreamUrl: string | null = null;

      await bloggerPage.setRequestInterception(true);
      bloggerPage.on('request', (req: HTTPRequest) => {
        req.continue();
      });

      bloggerPage.on('response', async (res: HTTPResponse) => {
        try {
          const resUrl = res.url();
          const contentType = res.headers()['content-type'] || '';

          if (
            (resUrl.includes('video.g') || resUrl.includes('blogger.com/video')) &&
      ...
      try { await bloggerPage.close(); } catch (_e) { /* already closed */ }
    }
  } catch (_e) {
    return null;
  }
 ...
 */
async function extractFiledonStream(page: Page): Promise<string | null> {
  try {
    ...
    const videoSrc = await page.evaluate(() => {
      ...
      if (video) {
        ...
        if (source) {
          ...
          if (ss && !ss.startsWith('blob:') && ss.startsWith('http')) return ss;
        }
      }
      ...
      for (const el of elements) {
        ...
        if (dataFile && dataFile.startsWith('http')) return dataFile;
        const dataSrc = el.getAttribute('data-src');
        if (dataSrc && dataSrc.startsWith('http') && /\.(mp4|m3u8|webm)/i.test(dataSrc)) return dataSrc;
        const dataVideo = el.getAttribute('data-video');
        if (dataVideo && dataVideo.startsWith('http')) return dataVideo;
      }
...
export default async function extractStream(iframeUrl: string): Promise<string | null> {
  // Cek cache dulu
  ...
  try {
    ...
    pg.on('response', async (res: HTTPResponse) => {
      if (streamUrl) return; // Already found
      try {
        const resUrl = res.url();
        const contentType = res.headers()['content-type'] || '';
        ...
        if (status >= 200 && status < 300 && (contentType.includes('json') || contentType.includes('javascript'))) {
          ...
          if (resUrl.includes('analytics') || resUrl.includes('tracking')) return;
    ...
    if (!streamUrl) {
      const src = await pg.evaluate(() => {
        ...
        for (const el of dataElements) {
          const dataFile = el.getAttribute('data-file');
          if (dataFile && dataFile.startsWith('http') && /\.(mp4|m3u8|webm)/i.test(dataFile)) return dataFile;
          const dataSrc = el.getAttribute('data-src');
          if (dataSrc && dataSrc.startsWith('http') && /\.(mp4|m3u8|webm)/i.test(dataSrc)) return dataSrc;
          const dataVideo = el.getAttribute('data-video');
          if (dataVideo && dataVideo.startsWith('http') && /\.(mp4|m3u8|webm)/i.test(dataVideo)) return dataVideo;
        }
        ...
        if ((window as any).jwplayer) {
          try {
            ...
            if (playlist && playlist.file) return playlist.file;
          } catch (_e) { /* ignore */ }
        }
        ...
        try {
          const src = await frame.evaluate(() => {
            ...
            if (dataEl) {
              ...
              if (df && df.startsWith('http') && /\.(mp4|m3u8|webm)/i.test(df)) return df;
            }
    ...
    if (streamUrl) cacheSet(iframeUrl, streamUrl);
    return streamUrl;
  } catch (e) {
    ...
    if (page) {
      try { await page.close(); } catch (_e) { /* page may already be closed */ }
    }
    releaseSlot();
  }
}
