  ...
  try {
    ...
    if (targetServer) {
      ...
      await page.evaluate((srv) => {
        const select = document.querySelector("#changeServer") as HTMLSelectElement;
        if (select) {
          select.value = srv;
...
function applySanitization(html: string, sanitize: boolean): string {
  if (sanitize) {
