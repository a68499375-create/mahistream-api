...
const subscribeKey = (animeId, episode) => episode ? `${animeId}|${episode}` : animeId;
const broadcast = (animeId, episode, event) => {
    const targets = [];
    const keys = [subscribeKey(animeId), subscribeKey(animeId, episode || undefined)];
    keys.forEach((k) => {
        const set = sseSubscribers.get(k);
        if (set)
            set.forEach((res) => targets.push(res));
    });
...
const commentsController = {
    ...
    async list(req, res) {
        ...
        try {
            ...
            const params = episode ? [animeId, episode] : [animeId];
            const tops = await queryDb(`SELECT c.*, u.display_name as user_name, u.picture as user_picture
         FROM comments c
         ...
         LIMIT ? OFFSET ?`, [...params, limit, offset]);
            ...
            if (topIds.length) {
                ...
                replies = await queryDb(`SELECT c.*, u.display_name as user_name, u.picture as user_picture
           FROM comments c
           LEFT JOIN users u ON u.id = c.user_id
           WHERE c.parent_id IN (${placeholders})
           ORDER BY c.created_at ASC`, topIds);
            }
            ...
            const out = tops.map((t) => ({ ...t, replies: byParent.get(t.id) || [] }));
    ...
    async create(req, res) {
        ...
        try {
            ...
            const result = await runDb(`INSERT INTO comments (user_id, anime_id, episode, parent_id, body) VALUES (?, ?, ?, ?, ?)`, [userId, animeId, episode || null, parentId || null, cleanBody]);
            const newId = result.lastID;
            const row = (await queryDb(`SELECT c.*, u.display_name as user_name, u.picture as user_picture
         FROM comments c LEFT JOIN users u ON u.id = c.user_id
         WHERE c.id = ?`, [newId]))[0];
            // Gamifikasi: +3 pts untuk comment baru.
            await runDb(`INSERT INTO user_points (user_id, points) VALUES (?, 3)
         ON CONFLICT(user_id) DO UPDATE SET points = points + 3, updated_at = CURRENT_TIMESTAMP`, [userId]);
            await runDb(`INSERT INTO points_log (user_id, action, delta, meta) VALUES (?, 'comment', 3, ?)`, [userId, JSON.stringify({ animeId, commentId: newId })]);
    ...
    async like(req, res) {
        ...
        try {
            ...
            const existing = await queryDb(`SELECT id FROM comment_likes WHERE user_id = ? AND comment_id = ?`, [userId, id]);
            ...
            if (row)
                broadcast(row.anime_id, row.episode, { type: "like", commentId: id, likes: row.likes });
    ...
    async remove(req, res) {
        ...
        try {
            ...
            if (own[0].user_id !== userId)
                return res.status(403).json(setPayload(res, { message: "Forbidden" }));
