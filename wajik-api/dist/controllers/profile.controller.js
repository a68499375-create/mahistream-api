...
const profileController = {
        ...
        try {
            ...
            if (row) {
                if (row.last_signin_date === today) {
                    ...
                    await runDb(`UPDATE signin_streak SET last_signin_date = ?, current_streak = ?, longest_streak = ?, total_signins = ? WHERE user_id = ?`, [today, newStreak, longest, totalSignins, userId]);
                }
            }
            else {
                await runDb(`INSERT INTO signin_streak (user_id, last_signin_date, current_streak, longest_streak, total_signins) VALUES (?, ?, 1, 1, 1)`, [userId, today]);
            }
            let pointsGained = 0;
            if (!alreadySignedToday) {
                ...
                if (newStreak > 0 && newStreak % 7 === 0)
                    pointsGained += 5;
                await runDb(`INSERT OR IGNORE INTO users (id, email, display_name) VALUES (?, ?, ?)`, [userId, userId, userId.split?.("@")?.[0] || userId]);
                await runDb(`INSERT INTO user_points (user_id, points) VALUES (?, ?)
           ON CONFLICT(user_id) DO UPDATE SET points = points + ?, updated_at = CURRENT_TIMESTAMP`, [userId, pointsGained, pointsGained]);
                await runDb(`INSERT INTO points_log (user_id, action, delta, meta) VALUES (?, 'signin', ?, ?)`, [userId, pointsGained, JSON.stringify({ streak: newStreak })]);
            }
    ...
    async points(req, res) {
        ...
        try {
            ...
            const log = await queryDb(`SELECT action, delta, meta, created_at FROM points_log WHERE user_id = ? ORDER BY created_at DESC LIMIT 20`, [userId]);
    ...
    async leaderboard(req, res) {
        ...
        try {
            const rows = await queryDb(`SELECT u.id as user_id, u.display_name, u.picture, p.points, s.current_streak
         FROM user_points p
    ...
    async me(req, res) {
        ...
        try {
            ...
            const counts = {
                history: (await queryDb(`SELECT COUNT(*) as c FROM history WHERE user_id = ?`, [userId]))[0]?.c || 0,
                bookmark: (await queryDb(`SELECT COUNT(*) as c FROM bookmarks WHERE user_id = ?`, [userId]))[0]?.c || 0,
                comments: (await queryDb(`SELECT COUNT(*) as c FROM comments WHERE user_id = ?`, [userId]))[0]?.c || 0,
            };
            return res.status(200).json(setPayload(res, {
                data: {
                    user,
    ...
    async update(req, res) {
        ...
        if (!userId)
            return res.status(400).json(setPayload(res, { message: "userId required" }));
        try {
            await runDb(`INSERT OR IGNORE INTO users (id, email, display_name) VALUES (?, ?, ?)`, [userId, userId, name || userId.split?.("@")?.[0] || userId]);
            const updates = [];
            const params = [];
            if (name !== undefined) {
                updates.push("display_name = ?");
                params.push(String(name).slice(0, 64));
            }
            if (bio !== undefined) {
                updates.push("bio = ?");
            ...
            if (picture !== undefined) {
                ...
                params.push(String(picture));
            }
            if (background_url !== undefined) {
                updates.push("background_url = ?");
                params.push(String(background_url));
            }
            if (updates.length > 0) {
                params.push(userId);
    ...
    async public(req, res) {
        ...
        if (!userId)
            return res.status(400).json(setPayload(res, { message: "userId required" }));
        try {
            const user = (await queryDb(`SELECT id, username, display_name, picture, background_url, bio FROM users WHERE id = ?`, [userId]))[0];
            if (!user)
            ...
            const points = (await queryDb(`SELECT points FROM user_points WHERE user_id = ?`, [userId]))[0] || { points: 0 };
            const recentWatched = await queryDb(`SELECT anime_id, title, poster_url, episode, watched_at FROM history WHERE user_id = ? ORDER BY watched_at DESC LIMIT 12`, [userId]);
