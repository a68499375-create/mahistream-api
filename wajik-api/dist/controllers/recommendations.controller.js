...
const recommendationsController = {
    ...
    async track(req, res) {
        ...
        try {
            ...
            await runDb(`INSERT INTO anime_stats (anime_id, title, poster_url, source, genres, view_count, last_viewed)
         ...
         ON CONFLICT(anime_id) DO UPDATE SET
           ...
           genres = COALESCE(NULLIF(excluded.genres, ''), genres)`, [animeId, title || "", poster || "", source || "kuramanime", genreStr]);
    ...
    async popular(req, res) {
        ...
        try {
            const rows = await queryDb(`SELECT anime_id, title, poster_url, source, genres, view_count
         FROM anime_stats
    ...
    async similar(req, res) {
        ...
        try {
            const seed = (await queryDb(`SELECT genres, source FROM anime_stats WHERE anime_id = ?`, [animeId]))[0];
            if (!seed || !seed.genres) {
                // Fallback: kalau seed tidak ada genre, kembalikan popular.
                const rows = await queryDb(`SELECT anime_id, title, poster_url, source, view_count
           FROM anime_stats WHERE anime_id != ?
           ORDER BY view_count DESC LIMIT ?`, [animeId, limit]);
                return res.status(200).json(setPayload(res, { data: { items: rows } }));
            }
    ...
    async forYou(req, res) {
        ...
        try {
            const history = await queryDb(`SELECT DISTINCT anime_id FROM history WHERE user_id = ? ORDER BY watched_at DESC LIMIT 5`, [userId]);
            const watchedIds = history.map((h) => h.anime_id);
            if (watchedIds.length === 0) {
                // Kalau belum ada history, fallback ke popular.
                const rows = await queryDb(`SELECT anime_id, title, poster_url, source, view_count
           FROM anime_stats ORDER BY view_count DESC LIMIT ?`, [limit]);
            ...
            const seeds = await queryDb(`SELECT genres FROM anime_stats WHERE anime_id IN (${placeholders})`, watchedIds);
            const genreSet = new Set();
            seeds.forEach((s) => {
                String(s.genres || "").split(",").forEach((g) => { const t = g.trim(); if (t)
                    genreSet.add(t); });
            });
            if (genreSet.size === 0) {
                const rows = await queryDb(`SELECT anime_id, title, poster_url, source, view_count
