...
const bypassAgent = new Agent({ connect: { lookup: lookupOverride } });
...
// yang dikirim client.
const pickRefererCandidates = (targetUrl, primary) => {
...
const proxyController = {
    async streamProxy(req, res) {
        try {
            ...
            const refererQ = req.query.referer || "";
            if (!targetUrl)
            ...
            let useBypass = false;
            try {
                useBypass = !!DNS_OVERRIDES[new URL(targetUrl).hostname];
            }
            catch (e) { }
            ...
            if (!response) {
                ...
                if (!res.headersSent)
                    res.status(lastStatus || 502).send(msg);
                return;
            }
            ...
            if (isManifest && response.body) {
                ...
                const wrap = (abs) => `${proxyBase}?url=${encodeURIComponent(abs)}${refererQ ? `&referer=${encodeURIComponent(refererQ)}` : ''}`;
                const rewritten = text.split('\n').map((line) => {
                    ...
                    if (trimmed.startsWith('#')) {
                        return line.replace(/URI="([^"]+)"/g, (_m, uri) => {
                            try {
                                return `URI="${wrap(new URL(uri, targetUrl).href)}"`;
                            }
                            catch {
                                return _m;
                            }
                    ...
                    try {
                        return wrap(new URL(trimmed, targetUrl).href);
                    }
                    catch {
                        return line;
                    }
