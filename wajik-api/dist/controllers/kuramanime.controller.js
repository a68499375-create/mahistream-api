...
const kuramanimeController = {
    async search(req, res) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Search timeout")), 20000));
    ...
    async latest(req, res) {
        ...
        try {
            ...
            const t = new Promise((_, reject) => setTimeout(() => reject(new Error("Latest timeout")), 20000));
    ...
    async episodeDetails(req, res) {
        ...
        const cached = global.__streamCache?.get(cacheKey);
        if (cached)
            return res.status(200).json(setPayload(res, { data: { details: cached } }));
        try {
            const details = await kuramanimeParser.parseEpisodeDetails(id);
            if (!details)
                return res.status(404).json(setPayload(res, { message: "Not found" }));
            if (!global.__streamCache)
                global.__streamCache = new Map();
            global.__streamCache.set(cacheKey, details);
            setTimeout(() => global.__streamCache?.delete(cacheKey), 5 * 60 * 1000);
            return res.status(200).json(setPayload(res, { data: { details } }));
        }
        catch (e) {
    ...
    async resolveStream(req, res) {
        ...
        try {
            ...
            let url = "";
            try {
                const details = await kuramanimeParser.parseEpisodeDetails(epId);
            ...
            if (!url) {
                try {
                    url = (await kuramanimeParser.parseStreamUrl(serverId)) || "";
                }
                catch { }
            }
            ...
            if (!global.__streamCache)
                global.__streamCache = new Map();
            global.__streamCache.set(cacheKey, url);
            setTimeout(() => global.__streamCache?.delete(cacheKey), 5 * 60 * 1000);
