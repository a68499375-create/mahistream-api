...
const nekopoiController = {
    ...
    async search(req, res) {
        const q = req.query.q;
        if (!q)
            return res.status(400).json(setPayload(res, { message: "query 'q' is required" }));
        const page = parseInt(req.query.page) || 1;
    ...
    async detail(req, res) {
        try {
            ...
            if (!url)
                return res.status(400).json(setPayload(res, { message: "url parameter is required" }));
        ...
        try {
            ...
            scripts.forEach((script) => {
                ...
                if (src) {
                    ...
                    const hasPopunder = lower.includes("window.open") &&
            ...
            const sizedTags = root.querySelectorAll("video, iframe, object, embed, source");
