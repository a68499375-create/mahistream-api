...
async function getSession(sessionName = DEFAULT_SESSION_NAME) {
    if (sessionId && sessionName === DEFAULT_SESSION_NAME)
        return sessionId;
    if (creating)
        return creating;
    creating = (async () => {
        ...
        const id = data.session || sessionName;
...
export default async function getHTMLViaFlareSolverr(url, maxTimeout = 55000, cookies, sessionName) {
    let lastError = "";
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        let data;
        try {
            const payload = { cmd: "request.get", url, maxTimeout, session };
            if (cookies)
...
export async function getHTMLViaFlareSolverrPost(url, postData, maxTimeout = 55000, cookies, sessionName) {
    let lastError = "";
    for (let attempt = 0; attempt < 2; attempt++) {
        let session;
        try {
        ...
        let data;
        try {
            const payload = { cmd: "request.post", url, postData, maxTimeout, session };
            if (cookies)
                payload.cookies = cookies;
            data = await post(payload);
        }
        catch (e) {
            if (!sessionName || sessionName === DEFAULT_SESSION_NAME)
                sessionId = null;
            lastError = e?.message || "request failed";
