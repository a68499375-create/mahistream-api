...
export default function errorHandler(err, req, res, next) {
    ...
    const statusCode = (typeof err.status === "number" && err.status < 500) ? err.status : 500;
    const message = err.message || "Terjadi kesalahan tak terduga";
    console.error("API ERROR:", err);
    res.status(statusCode).json(setPayload(res, {
        message: statusCode === 500 ? "Terjadi kesalahan tak terduga" : message,
    }));
}
